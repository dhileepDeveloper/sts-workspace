...
class BookServiceClientApplicationTests {
	...
	@Test
	public void getBookLoadBalanced()
	{
		String ingredientID = "1";
		Book book = restTemplate.getForObject("http://BOOK-SERVICE/book/{id}", Book.class,ingredientID);
		System.out.println("Book is " + book);
	}
	...
	{
		Book book = new Book();
		book.setAuthor("Orissa Balu");
		book.setName("I am Archaelogist");
	...
	@Test
	public void executeBookEntityWithURI()
	{
		Book book = new Book();
		book.setAuthor("Bruce Lee");
		book.setName("Jeet Kune Do");
		HttpEntity<Book> httpEntity = new HttpEntity<Book>(book, null);
		Book outputBook = restTemplate.execute("http://localhost:8081/book",HttpMethod.POST,httpEntity,Book.class);
		restTemplate.exe
		System.out.println("---------------------------");
		System.out.println("Book is " + entity.getBody());
		System.out.println("Status is " + entity.getStatusCode());
